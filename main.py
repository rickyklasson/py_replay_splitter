...
def get_video_files_from_folder(folder: Path) -> list[VideoFile]:
    ...
    print(f'---- Found {len(video_files)} recordings in folder: {folder}')
...
def get_overlapping_pairs(video_files_0: list[VideoFile],
    ...
    print(f'---- Found {len(overlaps)} overlapping recordings')
...
def extract_audio(video_path: Path, audio_path: Path):
    ...
    cmd = ['ffmpeg', '-i', str(video_path),
           ...
           '-stats',
           '-y', str(audio_path)]
    print(f'-- Merging {num_audio_streams} audio stream for synchronization...')
...
def sync_videos_by_audio(vid0: VideoFile, vid1: VideoFile):
    ...
    print(f'-- Audio offset between files: {offset:.3f}s')
...
def reencode_overlapping(vid0: VideoFile, vid1: VideoFile, out_folder: Path):
    ...
    print(f'---- Re-encoding overlapping files:\n\t\t{vid0.path}\n\t\t{vid1.path}')
    ...
    print(f'-- Re-encoding split screen view of recordings to file: {out_path}')
    cmd = ['ffmpeg',
           ...
           f'[1:a]atrim=start={vid1.trim_start}:end={vid1.trim_end},asetpts=PTS-STARTPTS[aout];',
           '-map', '[vout]',
           '-map', '[aout]',
           '-c:v', 'libx264',
           '-strict', 'experimental',
